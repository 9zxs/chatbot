...
with tab1:
    ...
    if not model_loaded:
        st.error("🚫 Chatbot unavailable due to model issues.")
        st.stop()
    ...
    with col1:
        user_input = st.text_input("Message", key="user_input", placeholder="Type here...")
    with col2:
    ...
    quick_qs = ["Admission deadline", "Tuition fees", "Available courses", "Scholarships"]
    ...
    if send_button and user_input.strip():
        ...
        response = random.choice(intent_to_responses.get(intent, ["Sorry, I don't understand."]))
    ...
    if st.session_state.messages:
        ...
        for idx, chat in enumerate(st.session_state.messages):
            ...
            <div class="bot-message">
                🤖 {chat['bot']}
                <div class="metadata">
                    🎯 Intent: {chat['intent']} | 📈 Confidence: {chat['confidence']:.2f}
                </div>
            </div>
            """
            # Feedback buttons
