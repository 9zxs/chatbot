...
if st.session_state.messages:
    ...
    for idx, chat in enumerate(st.session_state.messages):
        ...
        # Metadata
        confidence_color = (
            "🟢" if chat['confidence'] > 0.7 
            else "🟡" if chat['confidence'] > 0.5 
            else "🔴"
        )
        st.caption(f"🎯 Intent: {chat['intent']} | {confidence_color} Confidence: {chat['confidence']:.2f}")
    ...
    st.info("💬 Start a conversation! Ask me anything about the university.")
